...
(async()=>{
	...
	//
	let startOfWeek = moment().startOf('quarter');
	let tableType = "quarter";
	let lastWeek = moment(startOfWeek).subtract(1, 'quarters');
	...
	let filepath = `report-${tableType}-battery-${moment(lastWeek).year()}-${moment(lastWeek).quarter()}.xlsx`;
	...
	await my_report_log.findOrCreate({
		...
		defaults: {
			...
			report_index: `${moment(lastWeek).year()}年${moment(lastWeek).quarter()}季度`,
			report_path: filepath,
		}
	});
	process.exit();
