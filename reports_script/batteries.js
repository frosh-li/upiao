...
(async()=>{
	...
	let lastWeek = moment(startOfWeek).subtract(7, 'days');
	...
	let filepath = `report-week-battery-${moment(lastWeek).year()}-${moment(lastWeek).week()}.xlsx`;
	...
	await my_report_log.findOrCreate({
		...
		defaults: {
			...
			report_index: `${moment(lastWeek).year()}年${moment(lastWeek).week()}周`,
			report_path: filepath,
		}
	});
	process.exit();
