...
(async()=>{
	...
	startOfWeek = moment(startOfWeek).add(1, 'months');
	let tableType = "month";
	let lastWeek = moment(startOfWeek).subtract(1, 'months');
	...
	let filepath = `report-${tableType}-battery-${moment(lastWeek).year()}-${moment(lastWeek).month()}.xlsx`;
	...
	await my_report_log.findOrCreate({
		...
		defaults: {
			...
			report_index: `${moment(lastWeek).year()}年${moment(lastWeek).month()}月`,
			report_path: filepath,
		}
	});
	process.exit();
