...
class Service {
    ...
    */
    batchInsertCaution(data, insertHistory) {
        ...
        data.forEach(item => {
          let sql = `insert into my_alerts(type, code, time, current, climit) values("${item.type}","${item.code}","${item.time}",current="${item.current}","${item.climit}") where sn_key="${item.sn_key}"`;
          console.log(sql);
          sqls.push(sql);
          let sql_history = `insert into my_alerts_history(type, code, time, current, climit) values("${item.type}","${item.code}","${item.time}",current="${item.current}","${item.climit}") where sn_key="${item.sn_key}"`;
